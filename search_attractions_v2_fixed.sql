...
-- Create function for DOUBLE PRECISION rating column
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE NULL
        END as distance_km,
        -- Rating is already DOUBLE PRECISION
        COALESCE(l.rating, 0) as rating_numeric
    FROM locations l
    WHERE 
        l.type = 'attraction'
        ...
        -- Rating filters - now work since rating is DOUBLE PRECISION
        AND (min_rating IS NULL OR l.rating >= min_rating)
        AND (max_rating IS NULL OR l.rating <= max_rating)
