...
-- Create a new function with corrected SQL syntax
...
BEGIN
    ...
    SELECT 
        ...
        -- Rating conversion
        CASE 
            WHEN l.rating IS NOT NULL AND l.rating::TEXT ~ '^[0-9]+\.?[0-9]*$' THEN l.rating::DOUBLE PRECISION 
            ELSE 0 
    ...
    WHERE 
        ...
        AND (min_rating IS NULL OR (
            ...
            AND l.rating::DOUBLE PRECISION >= min_rating
        ))
        AND (max_rating IS NULL OR (
            l.rating IS NOT NULL 
            AND l.rating::TEXT ~ '^[0-9]+\.?[0-9]*$'
            AND l.rating::DOUBLE PRECISION <= max_rating
        ))
