...
-- Create a function that avoids regex entirely
...
DECLARE
    ...
    rating_as_number DOUBLE PRECISION;
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE NULL
        END as distance_km,
        -- Rating conversion - try to cast directly, catch exceptions
        CASE 
            WHEN l.rating IS NOT NULL AND l.rating != '' THEN 
                CASE 
                    WHEN l.rating::TEXT = '0' THEN 0::DOUBLE PRECISION
                    WHEN l.rating::TEXT = '1' THEN 1::DOUBLE PRECISION
                    WHEN l.rating::TEXT = '2' THEN 2::DOUBLE PRECISION
                    WHEN l.rating::TEXT = '3' THEN 3::DOUBLE PRECISION
                    WHEN l.rating::TEXT = '4' THEN 4::DOUBLE PRECISION
                    WHEN l.rating::TEXT = '5' THEN 5::DOUBLE PRECISION
                    WHEN l.rating::TEXT LIKE '%.%' THEN 
                        CASE 
                    ...
                    ELSE 
                        CASE 
                            WHEN l.rating::TEXT ~ '^[0-9]+$' THEN l.rating::TEXT::DOUBLE PRECISION
                            ELSE 0
                        END
                END
            ELSE 0 
    ...
    WHERE 
        ...
        -- Rating filters - no regex, just try to convert
