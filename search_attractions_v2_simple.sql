...
-- Create a simple function without regex operators
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE NULL
        END as distance_km,
        -- Simple rating conversion - just use 0 if can't convert
        COALESCE(
            CASE 
                WHEN l.rating IS NOT NULL AND length(trim(l.rating)) > 0 THEN 
                    CASE 
                        WHEN l.rating ~ '^[0-9]+\.?[0-9]*$' THEN l.rating::DOUBLE PRECISION
                        ELSE 0
                    END
                ELSE 0
            END, 0
    ...
    WHERE 
        ...
        -- Skip rating filters for now to avoid regex issues
